...
class PSDataset(Data.Dataset):
    def __init__(self, mat_dir, scale):
        self.scale = scale
    ...
    def __getitem__(self, index):
        ...
        pan = np.expand_dims(np.array(mat_data['PAN']), axis=0)
        gt = np.array(mat_data['HRMS']).transpose(-1, 0, 1)
        ms = np.array(mat_data['LRMS']).transpose(-1, 0, 1)
        
        pan    = imresize(pan, scale_factor=1/self.scale)
        gt    = imresize(gt, scale_factor=1/self.scale)

        ms    = imresize(ms, scale_factor=1/self.scale)

        # print(111111111111111111111111111111111111111111111111111111111111)
        # print(f" val pan {pan.shape} gt {gt.shape} ms {ms.shape} ")

        return torch.Tensor(ms), torch.Tensor(pan), torch.Tensor(gt)



class PSTestDataset(Data.Dataset):
    def __init__(self, mat_dir, scale):
        self.scale = scale
        self.mat_dir = mat_dir
        self.files = mat_dir
...
class PSMatDataset(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(f" pan {pan.shape} gt {gt.shape} ms {ms.shape} ")
...
def ergas(img_fake, img_real, scale=8):
...
def cc(img1, img2):
    ...
    print(f" img1 {img1.shape} img1 {img2.shape}")
    N,C,_,_ = img1.shape
